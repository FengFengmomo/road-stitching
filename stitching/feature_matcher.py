...
import cv2 as cv
import numpy as np
from stitching.refine.BestOf2NearestMatcher import BestOf2NearestMatcher
...
class FeatureMatcher:
    ...
    def __init__(
        ...
        elif matcher_type == "homography":
            self.matcher = cv.detail_BestOf2NearestRangeMatcher(int(range_width), **kwargs)
        elif matcher_type == "knnmatch":
            self.matcher = BestOf2NearestMatcher(ratio_thresh=0.8, ransac_thresh=3.0)
    ...
    def get_match_conf(match_conf, feature_detector_type):
        ...
        return match_conf # 如果是orb则 matchconf为0.3, 否则是0.65

    @staticmethod
    def get_default_match_conf(feature_detector_type):
        if feature_detector_type == "orb": # 如果是orb则 matchconf为0.3, 否则是0.65
            return 0.3
        return 0.65
